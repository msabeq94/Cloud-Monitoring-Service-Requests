...
foreach ($SearchAlertRuleName in $SearchAlertRuleNames) {
    $RM_AzSearchAlertRule = "https://management.azure.com/subscriptions/$($subscriptionID)/resourceGroups/$($RMResourceGroup)/providers/Microsoft.Insights/scheduledqueryrules/$($SearchAlertRuleName)?api-version=2021-08-01"
    try {
...
do {
 ...
 $header = @{
     ...
     if ([string]::IsNullOrEmpty($RMResourceGroup) -or $RMResourceGroup -eq "yes" -or $RMResourceGroup -eq "y") {
         $RMResourceGroupName = Read-Host "Enter the Resource Group name"
         $RMvmLocation = Read-Host "Do you want to cahnge virtual machine location? 
 Curent : $vmLocation 
 (yes/no) "
...
"@     
         $AzLogAlertRuleExistingConditionResourceGroupRG = $UpdateNEWRGScopev2RG   
         $AzLogAlertRuleExistingConditionResourceTypeRG = $NEWRTyScopeRG | ConvertTo-Json -Depth 10
...
"@
 ...
 } while ($RMResourceGroup -eq "yes" -or $RMResourceGroup -eq "y" -or [string]::IsNullOrEmpty($RMResourceGroup))
